...
frontDeskRoutes.post('/services', requireRole(Role.ADMIN, Role.RECEPTIONIST), (req, res, next) => frontDeskController.createService(req, res).catch(next));
frontDeskRoutes.post('/resources', requireRole(Role.ADMIN, Role.RECEPTIONIST), (req, res, next) => frontDeskController.createResource(req, res).catch(next));
frontDeskRoutes.post('/schedules', requireRole(Role.ADMIN, Role.RECEPTIONIST), (req, res, next) => frontDeskController.createSchedule(req, res).catch(next));
frontDeskRoutes.post('/schedule-exceptions', requireRole(Role.ADMIN, Role.RECEPTIONIST), (req, res, next) => frontDeskController.createException(req, res).catch(next));
