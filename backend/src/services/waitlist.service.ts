...
export const waitlistService = {
  ...
  async create(input: Create & { userId: string; role: Role }, audit: AuditContext) { await accessService.assertBranchAccess(input.userId, input.role, input.branchId); const { userId: _authorizationUserId, role: _authorizationRole, ...waitlistData } = input; void _authorizationUserId; void _authorizationRole; const entry = await waitlistRepository.create(waitlistData); await auditService.record({ ...audit, branchId: input.branchId }, { action: 'WAITLIST_CREATED', entity: 'WaitlistEntry', entityId: entry.id }); return entry; },
