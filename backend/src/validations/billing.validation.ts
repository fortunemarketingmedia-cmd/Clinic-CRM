...
export const patientPackageSchema = z.object({ patientId: z.string().min(1), packageMasterId: z.string().min(1), branchId: z.string().min(1), startDate: z.coerce.date().optional(), discount: z.coerce.number().min(0).default(0), paidAmount: z.coerce.number().min(0).default(0), notes: z.string().optional() });
