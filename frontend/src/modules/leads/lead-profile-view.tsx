...
import { ArrowLeft, CalendarDays, CheckSquare, Clock3, Flame, Phone, UserRound } from 'lucide-react';
...
export function LeadProfileView({ leadId }: { leadId: string }) {
  ...
  const recalculate = useMutation({ mutationFn: () => apiRequest(`/leads/${leadId}/recalculate-score`, { method: 'POST' }), onSuccess: () => client.invalidateQueries({ queryKey: ['lead-profile', leadId] }) });
  ...
  const scoreTone = lead.scoreCategory === 'HOT' ? 'bg-red-100 text-red-700' : lead.scoreCategory === 'WARM' ? 'bg-amber-100 text-amber-700' : lead.scoreCategory === 'UNQUALIFIED' ? 'bg-slate-100 text-slate-600' : 'bg-blue-100 text-blue-700';
  return <section className="space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><Link href="/leads" className="mb-2 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />Leads</Link><h1 className="text-2xl font-semibold">{lead.name}</h1><p className="text-sm text-muted-foreground">{lead.mobile} · {lead.branch?.name}</p></div><div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1 text-sm font-semibold ${scoreTone}`}>{lead.scoreCategory ?? 'COLD'} · {lead.leadScore ?? 0}</span><Button variant="secondary" onClick={() => recalculate.mutate()} disabled={recalculate.isPending}><Flame className="size-4" />Recalculate</Button></div></div>
    <div className="grid gap-4 xl:grid-cols-[1fr_340px]">
      ...
      <Card><h2 className="font-semibold">Change CRM stage</h2><div className="mt-3 space-y-2"><Select value={stage} onChange={(event) => setStage(event.target.value)}><option value="">Select next stage</option>{['ASSIGNED', 'ATTEMPTING_CONTACT', 'CONNECTED', 'QUALIFIED', 'APPOINTMENT_PROPOSED', 'NURTURING', 'LOST', 'DISQUALIFIED'].map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</Select>{['QUALIFIED', 'LOST', 'DISQUALIFIED'].includes(stage) ? <Input placeholder={stage === 'QUALIFIED' ? 'Qualification notes' : 'Required reason'} value={stageNotes} onChange={(event) => setStageNotes(event.target.value)} /> : null}{stage === 'QUALIFIED' ? <Input type="number" min="0" max="100" value={manualScore} onChange={(event) => setManualScore(event.target.value)} /> : null}{transition.isError ? <p className="text-xs text-red-600">{transition.error.message}</p> : null}<Button className="w-full" disabled={!stage || (['QUALIFIED', 'LOST', 'DISQUALIFIED'].includes(stage) && !stageNotes) || transition.isPending} onClick={() => transition.mutate()}>Apply stage</Button></div></Card>
