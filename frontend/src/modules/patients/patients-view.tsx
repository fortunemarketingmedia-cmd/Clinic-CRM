...
const serviceLabel = (service: ClinicService) => `${service.category ? `${formatEnum(service.category)} - ` : ''}${service.name}`;
...
function TreatmentCreateModal({ mode, patient, appointments, services, packages, form, prescriptionEnabled, setPrescriptionEnabled, prescription, setPrescription, medicines, file, setFile, mutation, onAppointmentChange, onClose }: { mode: 'create' | 'edit'; patient: Patient; appointments: Appointment[]; services: ClinicService[]; packages: TreatmentPackage[]; form: UseFormReturn<TreatmentValues>; prescriptionEnabled: boolean; setPrescriptionEnabled: (value: boolean) => void; prescription: PrescriptionMedicine[]; setPrescription: (items: PrescriptionMedicine[]) => void; medicines: MedicineOption[]; file: File | null; setFile: (file: File | null) => void; mutation: MutationFor<TreatmentValues>; onAppointmentChange: (id: string) => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-2 sm:p-4"><Card className="mx-auto max-w-5xl"><ModalHeader title={mode === 'edit' ? 'Edit Treatment' : 'Create Treatment'} description={`${mode === 'edit' ? 'Update' : 'Record'} treatment details for ${patient.fullName}.`} onClose={onClose} /><form className="space-y-6" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
    <FormSection number="1" title="Visit & treatment"><div className="grid gap-3 md:grid-cols-2"><Field label="Appointment / room booking (optional)"><Select {...form.register('appointmentId')} onChange={(event) => onAppointmentChange(event.target.value)}><option value="">Not linked to an appointment</option>{availableAppointments.map((item) => <option key={item.id} value={item.id}>{formatDateTime(item.appointmentAt)} - {item.service?.name ?? (item.appointmentType === 'VIDEO_CONSULTATION' ? 'Video consultation' : item.resourceType === 'TREATMENT_ROOM' ? `Room ${item.roomNumber ?? ''}` : 'Consultation')} - {formatEnum(item.status)}</option>)}</Select></Field><Field label="Treatment type"><Select {...form.register('treatmentType')}><option value="CONSULTATION">Consultation / checkup</option><option value="VIDEO_CONSULTATION">Video consultation</option><option value="TREATMENT_ROOM">Treatment room session</option><option value="PROCEDURE">Procedure</option><option value="FOLLOW_UP">Follow-up</option><option value="OTHER">Other</option></Select></Field><Field label="Date & time" required><Input type="datetime-local" {...form.register('visitDate')} /></Field><Field label="Treatment name" required><Select value={selectedTreatmentName ?? ''} onChange={(event) => { const service = activeServices.find((item) => item.name === event.target.value); form.setValue('treatmentTaken', event.target.value); form.setValue('treatmentSuggested', event.target.value); if (service?.resourceType === 'TREATMENT_ROOM') form.setValue('treatmentType', 'TREATMENT_ROOM'); }}><option value="">Select treatment name</option>{activeServices.map((service) => <option key={service.id} value={service.name}>{serviceLabel(service)}</option>)}{selectedTreatmentName && !activeServices.some((service) => service.name === selectedTreatmentName) ? <option value={selectedTreatmentName}>{selectedTreatmentName}</option> : null}</Select></Field><Field label="Linked package"><Select {...form.register('packageId')}><option value="">No package / pay per visit</option>{packageOptions.map((item) => <option key={item.id} value={item.id}>{item.name} - {item.totalSessions - item.completedSessions}/{item.totalSessions} left</option>)}</Select></Field><Field label="Chief complaint"><Input placeholder="Reason for visit" {...form.register('chiefComplaint')} /></Field><Field label="Diagnosis"><Input placeholder="Clinical diagnosis" {...form.register('diagnosis')} /></Field><Field label="Follow-up date"><Input type="datetime-local" {...form.register('followupDate')} /></Field><Field label="Clinical notes"><Input placeholder="Observations, advice, precautions" {...form.register('notes')} /></Field></div></FormSection>

    <FormSection number="2" title="Prescription"><label className="flex cursor-pointer items-center gap-3 rounded-lg border border-border bg-surface p-3"><input type="checkbox" checked={prescriptionEnabled} onChange={(event) => setPrescriptionEnabled(event.target.checked)} /><span><span className="block text-sm font-medium">Generate a prescription</span><span className="block text-xs text-muted-foreground">Add medicine, dose, frequency, duration, and instructions. It can be printed from the treatment card.</span></span></label>{prescriptionEnabled ? <div className="mt-4 space-y-3"><datalist id="medicine-catalog"><option value="Paracetamol" /><option value="Cetirizine" /><option value="Amoxicillin" /><option value="Azithromycin" /><option value="Doxycycline" /><option value="Isotretinoin" /><option value="Tretinoin cream" /><option value="Clindamycin gel" /><option value="Minoxidil" /><option value="Vitamin D3" />{medicines.map((medicine) => <option key={medicine.id} value={medicine.name}>{[medicine.strength, medicine.form].filter(Boolean).join(' - ')}</option>)}</datalist>{prescription.map((item, index) => <div key={index} className="grid gap-2 rounded-lg border border-border p-3 md:grid-cols-[1.5fr_1fr_1fr_1fr_1.5fr_auto]"><Input list="medicine-catalog" placeholder="Medicine" value={item.medicine} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'medicine', event.target.value)} /><Select value={item.dosage} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'dosage', event.target.value)}><option value="">Dose</option><option>1/2 tablet</option><option>1 tablet</option><option>2 tablets</option><option>5 ml</option><option>10 ml</option><option>Apply thin layer</option><option>As directed</option></Select><Select value={item.frequency} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'frequency', event.target.value)}><option value="">Frequency</option><option>Once daily</option><option>Twice daily</option><option>Three times daily</option><option>At bedtime</option><option>As needed</option></Select><Input placeholder="Duration" value={item.duration} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'duration', event.target.value)} /><Input placeholder="Before/after food, precautions" value={item.instructions} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'instructions', event.target.value)} /><Button type="button" variant="ghost" className="w-10 px-0" aria-label="Remove medicine" onClick={() => setPrescription(prescription.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="size-4" /></Button></div>)}<Button type="button" variant="secondary" onClick={() => setPrescription([...prescription, blankMedicine()])}><Plus className="size-4" />Add Medicine</Button></div> : null}</FormSection>
