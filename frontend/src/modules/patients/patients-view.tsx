...
const serviceLabel = (service: ClinicService) => `${service.category ? `${formatEnum(service.category)} Â· ` : ''}${service.name}`;
...
export function PatientsView() {
  ...
  const [treatmentMode, setTreatmentMode] = useState<'create' | 'edit'>('create');
  ...
  const medicinesQuery = useQuery({
    queryKey: ['clinical-medicines'],
    queryFn: () => apiRequest<{ data: MedicineOption[] }>('/clinical/medicines'),
    enabled: hasHydrated && Boolean(session),
  });
  ...
  const servicesQuery = useQuery({
    queryKey: ['client-directory-services', selectedPatient?.branchId ?? activeBranchId],
    queryFn: () => apiRequest<{ data: ClinicService[] }>(`/front-desk/services?branchId=${selectedPatient?.branchId ?? activeBranchId}`),
    enabled: Boolean(selectedPatient?.branchId ?? activeBranchId),
  });
  const patientPackagesQuery = useQuery({
    queryKey: ['client-directory-patient-packages', selectedPatient?.id],
    queryFn: () => apiRequest<{ data: TreatmentPackage[] }>(`/billing/patient-packages?patientId=${selectedPatient?.id}`),
    enabled: Boolean(selectedPatient?.id),
  });
  ...
  const createTreatment = useMutation({
    mutationFn: async (values: z.infer<typeof treatmentSchema>) => {
      ...
      const treatment = await apiRequest<{ data: PatientSession }>(`/patients/${selectedPatient.id}/sessions`, {
        ...
        body: JSON.stringify({
          ...
          doctorConsulted: values.doctorConsulted || 'Revive Clinic',
    ...
    onSuccess: () => {
      treatmentForm.reset({ appointmentId: '', treatmentType: 'CONSULTATION', visitDate: '', doctorConsulted: '', packageId: '' });
  ...
  const updateTreatment = useMutation({
    mutationFn: async (values: z.infer<typeof treatmentSchema>) => {
      if (!selectedPatient || !selectedTreatment) throw new Error('Select a treatment first');
      const validPrescription = prescriptionEnabled
        ? prescription.filter((item) => item.medicine && item.dosage && item.frequency && item.duration)
        : [];
      if (prescriptionEnabled && !validPrescription.length) throw new Error('Complete at least one prescription medicine');
      return apiRequest<{ data: PatientSession }>(`/patients/${selectedPatient.id}/sessions/${selectedTreatment.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
          appointmentId: values.appointmentId || undefined,
          treatmentType: values.treatmentType,
  ...
  const deleteTreatment = useMutation({
    mutationFn: async (sessionId: string) => {
      if (!selectedPatient) throw new Error('Select a client first');
      return apiRequest(`/patients/${selectedPatient.id}/sessions/${sessionId}`, { method: 'DELETE' });
    },
    onSuccess: () => {
      setSelectedTreatment(null);
      queryClient.invalidateQueries({ queryKey: ['patient-sessions'] });
  ...
  const openTreatmentForm = () => {
    setTreatmentMode('create');
    setSelectedTreatment(null);
    treatmentForm.setValue('visitDate', toLocalDateTime(new Date()));
    treatmentForm.setValue('appointmentId', '');
    treatmentForm.setValue('treatmentType', 'CONSULTATION');
    treatmentForm.setValue('doctorConsulted', '');
    treatmentForm.setValue('chiefComplaint', '');
    treatmentForm.setValue('diagnosis', '');
    treatmentForm.setValue('treatmentSuggested', '');
    treatmentForm.setValue('treatmentTaken', '');
    treatmentForm.setValue('notes', '');
    treatmentForm.setValue('followupDate', '');
    treatmentForm.setValue('packageId', '');
    setPrescriptionEnabled(false);
    setPrescription([blankMedicine()]);
    setShowCreateTreatment(true);
  };

  const openEditTreatment = (treatment: PatientSession) => {
    setTreatmentMode('edit');
    setSelectedTreatment(treatment);
    treatmentForm.reset({
      appointmentId: treatment.appointmentId ?? '',
      treatmentType: treatment.treatmentType,
      visitDate: toLocalDateTime(new Date(treatment.visitDate)),
      doctorConsulted: treatment.doctorConsulted ?? '',
      chiefComplaint: treatment.chiefComplaint ?? '',
      diagnosis: treatment.diagnosis ?? '',
      treatmentSuggested: treatment.treatmentSuggested ?? '',
      treatmentTaken: treatment.treatmentTaken ?? '',
      notes: treatment.notes ?? '',
      followupDate: treatment.followupDate ? toLocalDateTime(new Date(treatment.followupDate)) : '',
      packageId: treatment.packageId ?? '',
    });
    setPrescriptionEnabled(Boolean(treatment.prescription?.length));
    setPrescription(treatment.prescription?.length ? treatment.prescription : [blankMedicine()]);
    setShowCreateTreatment(true);
  };
  ...
  const selectAppointment = (appointmentId: string) => {
    ...
    treatmentForm.setValue('doctorConsulted', appointment.doctor?.name ?? treatmentForm.getValues('doctorConsulted') ?? '');
    treatmentForm.setValue('treatmentSuggested', appointment.service?.name ?? treatmentForm.getValues('treatmentSuggested') ?? '');
    treatmentForm.setValue('treatmentTaken', appointment.service?.name ?? (appointment.resourceType === 'TREATMENT_ROOM' ? `Treatment room${appointment.roomNumber ? ` ${appointment.roomNumber}` : ''}` : appointment.appointmentType === 'VIDEO_CONSULTATION' ? 'Video consultation' : 'Consultation'));
  };

  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><h1 className="text-2xl font-semibold">Client Directory</h1><p className="text-sm text-muted-foreground">All registered clients, including consultation, checkup, treatment, prescription, and file records.</p></div>
        <Button type="button" onClick={() => setShowCreatePatient(true)}><Plus className="size-4" />Create Client</Button>
      </div>

      <Card>
        <div className="mb-4">
          <div className="relative"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-muted-foreground" /><Input className="pl-9" placeholder="Search client name, mobile, or client no" value={search} onChange={(event) => setSearch(event.target.value)} /></div>
        </div>
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full border-collapse text-left text-sm"><thead className="bg-muted text-muted-foreground"><tr><th className="px-4 py-3 font-medium">Client</th><th className="px-4 py-3 font-medium">Mobile</th><th className="px-4 py-3 font-medium">Branch</th><th className="px-4 py-3 font-medium">Source</th><th className="px-4 py-3 font-medium">Created</th><th className="px-4 py-3 font-medium">Action</th></tr></thead>
            <tbody>{patientsQuery.isLoading ? Array.from({ length: 6 }, (_, row) => <tr key={row} className="border-t border-border">{Array.from({ length: 6 }, (_, column) => <td key={column} className="px-4 py-4"><Skeleton className={column === 0 ? 'h-5 w-32' : 'h-4 w-24'} /></td>)}</tr>) : uniquePatients.map((patient) => <tr key={patient.id} className="cursor-pointer border-t border-border hover:bg-muted/50" onClick={() => { setSelectedPatientId(patient.id); setActivePatientTab('details'); }}><td className="px-4 py-3"><div className="font-medium">{patient.fullName}</div><div className="text-xs text-muted-foreground">{patient.patientNo}</div></td><td className="px-4 py-3">{patient.mobile}</td><td className="px-4 py-3">{patient.branch?.name}</td><td className="px-4 py-3">{formatEnum(patient.lead?.source)}</td><td className="px-4 py-3">{formatDateTime(patient.createdAt)}</td><td className="px-4 py-3"><Button type="button" variant="secondary" onClick={(event) => { event.stopPropagation(); setSelectedPatientId(patient.id); setActivePatientTab('details'); }}>View Profile</Button></td></tr>)}
              {!patientsQuery.isLoading && uniquePatients.length === 0 ? <tr><td className="px-4 py-8 text-center text-muted-foreground" colSpan={6}>No clients found.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </Card>
          ...
          <Card className="mx-auto min-h-[calc(100vh-2rem)] max-w-[1400px]">
            <div className="space-y-6">
              <div className="sticky -top-6 z-10 flex flex-col gap-3 border-b border-border bg-surface py-3 sm:flex-row sm:items-start sm:justify-between">
                <div><div className="mb-1 flex items-center gap-2"><span className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary">{selectedPatient.patientNo}</span><span className="text-xs text-muted-foreground">Created {formatDateTime(selectedPatient.createdAt)}</span></div><h2 className="text-2xl font-semibold">{selectedPatient.fullName}</h2><p className="text-sm text-muted-foreground">{selectedPatient.mobile} Â· {selectedPatient.branch?.name} Â· Source: {formatEnum(selectedPatient.lead?.source)}</p></div>
                <div className="flex gap-2"><Link href={`/patients/${selectedPatient.id}`} className="inline-flex h-10 items-center justify-center rounded-md border border-border bg-surface px-4 text-sm font-medium"><UserRound className="mr-2 size-4" />Full profile</Link><Button type="button" variant="secondary" onClick={() => setIsEditingPatient((value) => !value)}><Pencil className="size-4" />{isEditingPatient ? 'Close' : 'Edit'}</Button><Button type="button" variant="secondary" className="w-10 px-0" aria-label="Close profile" onClick={() => { setSelectedPatientId(null); setIsEditingPatient(false); }}><X className="size-4" /></Button></div>
              </div>
              ...
              <div className="flex gap-2 overflow-x-auto border-b border-border pb-3"><Button type="button" variant={activePatientTab === 'details' ? 'primary' : 'secondary'} onClick={() => setActivePatientTab('details')}><UserRound className="size-4" />Overview</Button><Button type="button" variant={activePatientTab === 'treatments' ? 'primary' : 'secondary'} onClick={() => setActivePatientTab('treatments')}><Stethoscope className="size-4" />Treatments</Button></div>

              {activePatientTab === 'details' ? (
                <div className="space-y-5">
                  <SectionHeader title="Client information" description="Basic identity, contact, and clinic registration details." />
                  ...
                  <div className="flex items-end justify-between gap-3"><SectionHeader title="Medical background" description="Only the intake details needed for clinic follow-up and treatment." /><Button type="button" variant="secondary" onClick={() => setIsEditingMedical((value) => !value)}><Pencil className="size-4" />{isEditingMedical ? 'Close' : 'Edit medical'}</Button></div>
              ...
              {activePatientTab === 'treatments' ? (
                <div className="space-y-6">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><SectionHeader title="Treatments" description="Consultations, checkups, procedures, prescriptions, and files in one place." /><Button type="button" onClick={openTreatmentForm}><ClipboardPlus className="size-4" />Add treatment</Button></div>
      ...
      {showCreateTreatment && selectedPatient ? <TreatmentCreateModal mode={treatmentMode} patient={selectedPatient} appointments={visitsQuery.data?.data ?? []} services={servicesQuery.data?.data ?? []} packages={patientPackagesQuery.data?.data ?? []} form={treatmentForm} prescriptionEnabled={prescriptionEnabled} setPrescriptionEnabled={setPrescriptionEnabled} prescription={prescription} setPrescription={setPrescription} medicines={medicinesQuery.data?.data ?? []} file={treatmentFile} setFile={setTreatmentFile} mutation={treatmentMode === 'edit' ? updateTreatment : createTreatment} onAppointmentChange={selectAppointment} onClose={() => setShowCreateTreatment(false)} /> : null}
      {selectedTreatment && selectedPatient && !showCreateTreatment ? <TreatmentDetailModal patient={selectedPatient} treatment={selectedTreatment} isDeleting={deleteTreatment.isPending} onEdit={() => openEditTreatment(selectedTreatment)} onDelete={() => { if (window.confirm('Delete this treatment record? This cannot be undone.')) deleteTreatment.mutate(selectedTreatment.id); }} onClose={() => setSelectedTreatment(null)} /> : null}
    </section>
  );
}
...
function TreatmentCreateModal({ mode, patient, appointments, services, packages, form, prescriptionEnabled, setPrescriptionEnabled, prescription, setPrescription, medicines, file, setFile, mutation, onAppointmentChange, onClose }: { mode: 'create' | 'edit'; patient: Patient; appointments: Appointment[]; services: ClinicService[]; packages: TreatmentPackage[]; form: UseFormReturn<TreatmentValues>; prescriptionEnabled: boolean; setPrescriptionEnabled: (value: boolean) => void; prescription: PrescriptionMedicine[]; setPrescription: (items: PrescriptionMedicine[]) => void; medicines: MedicineOption[]; file: File | null; setFile: (file: File | null) => void; mutation: MutationFor<TreatmentValues>; onAppointmentChange: (id: string) => void; onClose: () => void }) {
  ...
  const packageOptions = packages.filter((item) => item.totalSessions > item.completedSessions);
  const selectedTreatmentName = form.watch('treatmentTaken');
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-2 sm:p-4"><Card className="mx-auto max-w-5xl"><ModalHeader title={mode === 'edit' ? 'Edit Treatment' : 'Create Treatment'} description={`${mode === 'edit' ? 'Update' : 'Record'} treatment details for ${patient.fullName}.`} onClose={onClose} /><form className="space-y-6" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
    <FormSection number="1" title="Visit & treatment"><div className="grid gap-3 md:grid-cols-2"><Field label="Appointment / room booking (optional)"><Select {...form.register('appointmentId')} onChange={(event) => onAppointmentChange(event.target.value)}><option value="">Not linked to an appointment</option>{availableAppointments.map((item) => <option key={item.id} value={item.id}>{formatDateTime(item.appointmentAt)} Â· {item.service?.name ?? (item.appointmentType === 'VIDEO_CONSULTATION' ? 'Video consultation' : item.resourceType === 'TREATMENT_ROOM' ? `Room ${item.roomNumber ?? ''}` : 'Consultation')} Â· {formatEnum(item.status)}</option>)}</Select></Field><Field label="Treatment type"><Select {...form.register('treatmentType')}><option value="CONSULTATION">Consultation / checkup</option><option value="VIDEO_CONSULTATION">Video consultation</option><option value="TREATMENT_ROOM">Treatment room session</option><option value="PROCEDURE">Procedure</option><option value="FOLLOW_UP">Follow-up</option><option value="OTHER">Other</option></Select></Field><Field label="Date & time" required><Input type="datetime-local" {...form.register('visitDate')} /></Field><Field label="Treatment name" required><Select value={selectedTreatmentName ?? ''} onChange={(event) => { const service = activeServices.find((item) => item.name === event.target.value); form.setValue('treatmentTaken', event.target.value); form.setValue('treatmentSuggested', event.target.value); if (service?.resourceType === 'TREATMENT_ROOM') form.setValue('treatmentType', 'TREATMENT_ROOM'); }}><option value="">Select treatment name</option>{activeServices.map((service) => <option key={service.id} value={service.name}>{serviceLabel(service)}</option>)}{selectedTreatmentName && !activeServices.some((service) => service.name === selectedTreatmentName) ? <option value={selectedTreatmentName}>{selectedTreatmentName}</option> : null}</Select></Field><Field label="Linked package"><Select {...form.register('packageId')}><option value="">No package / pay per visit</option>{packageOptions.map((item) => <option key={item.id} value={item.id}>{item.name} Â· {item.totalSessions - item.completedSessions}/{item.totalSessions} left</option>)}</Select></Field><Field label="Chief complaint"><Input placeholder="Reason for visit" {...form.register('chiefComplaint')} /></Field><Field label="Diagnosis"><Input placeholder="Clinical diagnosis" {...form.register('diagnosis')} /></Field><Field label="Follow-up date"><Input type="datetime-local" {...form.register('followupDate')} /></Field><Field label="Clinical notes"><Input placeholder="Observations, advice, precautions" {...form.register('notes')} /></Field></div></FormSection>

    <FormSection number="2" title="Prescription"><label className="flex cursor-pointer items-center gap-3 rounded-lg border border-border bg-surface p-3"><input type="checkbox" checked={prescriptionEnabled} onChange={(event) => setPrescriptionEnabled(event.target.checked)} /><span><span className="block text-sm font-medium">Generate a prescription</span><span className="block text-xs text-muted-foreground">Add medicine, dose, frequency, duration, and instructions. It can be printed from the treatment card.</span></span></label>{prescriptionEnabled ? <div className="mt-4 space-y-3"><datalist id="medicine-catalog"><option value="Paracetamol" /><option value="Cetirizine" /><option value="Amoxicillin" /><option value="Azithromycin" /><option value="Doxycycline" /><option value="Isotretinoin" /><option value="Tretinoin cream" /><option value="Clindamycin gel" /><option value="Minoxidil" /><option value="Vitamin D3" />{medicines.map((medicine) => <option key={medicine.id} value={medicine.name}>{[medicine.strength, medicine.form].filter(Boolean).join(' Â· ')}</option>)}</datalist>{prescription.map((item, index) => <div key={index} className="grid gap-2 rounded-lg border border-border p-3 md:grid-cols-[1.5fr_1fr_1fr_1fr_1.5fr_auto]"><Input list="medicine-catalog" placeholder="Medicine" value={item.medicine} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'medicine', event.target.value)} /><Select value={item.dosage} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'dosage', event.target.value)}><option value="">Dose</option><option>Â½ tablet</option><option>1 tablet</option><option>2 tablets</option><option>5 ml</option><option>10 ml</option><option>Apply thin layer</option><option>As directed</option></Select><Select value={item.frequency} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'frequency', event.target.value)}><option value="">Frequency</option><option>Once daily</option><option>Twice daily</option><option>Three times daily</option><option>At bedtime</option><option>As needed</option></Select><Input placeholder="Duration" value={item.duration} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'duration', event.target.value)} /><Input placeholder="Before/after food, precautions" value={item.instructions} onChange={(event) => updateMedicine(prescription, setPrescription, index, 'instructions', event.target.value)} /><Button type="button" variant="ghost" className="w-10 px-0" aria-label="Remove medicine" onClick={() => setPrescription(prescription.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="size-4" /></Button></div>)}<Button type="button" variant="secondary" onClick={() => setPrescription([...prescription, blankMedicine()])}><Plus className="size-4" />Add Medicine</Button></div> : null}</FormSection>
...
function printPrescription(patient: Patient, treatment: PatientSession) {
  ...
  const logoUrl = `${window.location.origin}/revive-logo.png`;
  const clinicAddress = patient.branch?.address ?? 'Nashik Road, Nashik';
  const clinicPhone = patient.branch?.phone ?? '+91-0000000000';
  const clinicHours = 'Mon-Sat, 10:00 AM-7:00 PM';
  ...
  printWindow.document.write(`<!doctype html><html><head><title>Prescription - ${escapeHtml(patient.fullName)}</title><style>body{font:14px Arial,sans-serif;color:#241417;margin:32px}.letterhead{display:flex;gap:22px;align-items:center;border-bottom:3px solid #ef2b32;padding-bottom:16px}.logo{width:88px;height:88px;object-fit:contain}.brand{font-size:28px;font-weight:800;color:#ef2b32;margin-bottom:4px}.clinic{color:#5f464b;line-height:1.45}.doc-title{display:flex;justify-content:space-between;align-items:center;margin:22px 0 14px}.doc-title h2{margin:0;font-size:18px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:10px 26px;border:1px solid #ead6d0;border-radius:12px;padding:16px;margin-bottom:22px}.rx{font-size:32px;color:#ef2b32;font-weight:700;margin:16px 0 8px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ead6d0;padding:10px;text-align:left;vertical-align:top}th{background:#fff3ed;color:#5f3035}.note{margin-top:18px;line-height:1.5}.footer{margin-top:70px;display:flex;justify-content:space-between;gap:24px}.signature{text-align:right;min-width:220px;border-top:1px solid #241417;padding-top:8px}.muted{color:#6b5b5e;font-size:12px}@media print{body{margin:18px}.no-print{display:none}}</style></head><body><div class="letterhead"><img class="logo" src="${logoUrl}" alt="Revive Clinic logo" /><div><div class="brand">Revive Clinic</div><div class="clinic">${escapeHtml(clinicAddress)}<br/>Phone: ${escapeHtml(clinicPhone)}<br/>Timings: ${escapeHtml(clinicHours)}${patient.branch?.name ? `<br/>Branch: ${escapeHtml(patient.branch.name)}` : ''}</div></div></div><div class="doc-title"><h2>Medical Prescription</h2><span class="muted">Generated ${escapeHtml(formatDateTime(new Date()))}</span></div><div class="meta"><div><strong>Client:</strong> ${escapeHtml(patient.fullName)}</div><div><strong>Client No:</strong> ${escapeHtml(patient.patientNo)}</div><div><strong>Mobile:</strong> ${escapeHtml(patient.mobile)}</div><div><strong>Date:</strong> ${escapeHtml(formatDateTime(treatment.visitDate))}</div><div><strong>Doctor / Provider:</strong> ${escapeHtml(treatment.doctorConsulted)}</div><div><strong>Diagnosis:</strong> ${escapeHtml(treatment.diagnosis) || '-'}</div></div><div class="rx">Rx</div><table><thead><tr><th>#</th><th>Medicine</th><th>Dose</th><th>Frequency</th><th>Duration</th><th>Instructions</th></tr></thead><tbody>${rows}</tbody></table>${treatment.notes ? `<p class="note"><strong>Advice / Notes:</strong> ${escapeHtml(treatment.notes)}</p>` : ''}${treatment.followupDate ? `<p class="note"><strong>Follow-up:</strong> ${escapeHtml(formatDateTime(treatment.followupDate))}</p>` : ''}<div class="footer"><span class="muted">This prescription belongs to ${escapeHtml(patient.fullName)} and was generated from Revive Clinic CRM.</span><span class="signature">Doctor's signature</span></div><script>window.onload=()=>window.print()</script></body></html>`);
