...
const billingRoles = ['ADMIN', 'RECEPTIONIST'];
const operationalRoles = ['ADMIN', 'RECEPTIONIST'];
...
export function Patient360View({ patientId, embedded = false }: { patientId: string; embedded?: boolean }) {
  ...
  const canBill = Boolean(session && billingRoles.includes(session.user.role));
  const canOperate = Boolean(session && operationalRoles.includes(session.user.role));
  ...
  return <section className="space-y-5">
    ...
    {tab === 'Appointments' ? <AppointmentsPanel patient={patient} canEdit={canOperate} invalidate={invalidate} /> : null}
    {tab === 'Care History' ? <CareHistoryPanel patient={patient} /> : null}
    {tab === 'Billing' ? <BillingPanel patient={patient} canBill={canBill} invalidate={invalidate} /> : null}
...
const emptyPrescriptionItem = (name = ''): PrescriptionDraftItem => ({ medicineName: name, strength: '', dosage: '', frequency: '', duration: '', route: '', timing: '', instructions: '' });
function ClinicalComposer({ kind, patient, staff, defaultDoctorId, onClose, onSaved }: { kind: 'encounter' | 'prescription'; patient: Patient360; staff: StaffMember[]; resources: ClinicResource[]; defaultDoctorId?: string; onClose: () => void; onSaved: () => void }) {
  ...
  const updateItem = (index: number, key: keyof PrescriptionDraftItem, value: string) => setPrescriptionItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value } : item));
  const removeItem = (index: number) => setPrescriptionItems((current) => current.filter((_, itemIndex) => itemIndex !== index));
  const selectMedicine = (medicineId: string) => {
    ...
    setPrescriptionItems((current) => [...current, { medicineId: selected.id, medicineName: selected.name, strength: selected.strength ?? '', dosage: topical ? 'Apply a thin layer' : '', frequency: '', duration: '', route: topical ? 'Topical' : '', timing: '', instructions: '' }]);
    setMedicineSearch('');
  };
  const addManualMedicine = () => {
    if (!manualMedicineName.trim()) return;
    setPrescriptionItems((current) => [...current, emptyPrescriptionItem(manualMedicineName.trim())]);
  ...
  const mutation = useMutation({ mutationFn: async () => {
    ...
    const incomplete = prescriptionItems.find((item) => !item.dosage || !item.frequency || !item.duration);
    if (incomplete) throw new Error(`Complete dose, frequency, and duration for ${incomplete.medicineName || 'the medicine'} before saving`);
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-4"><Card className={`mx-auto w-full ${kind === 'prescription' ? 'max-w-4xl' : 'max-w-2xl'}`}><div className="flex justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary">{kind === 'prescription' ? 'Clinical prescription' : 'Clinical encounter'}</p><h2 className="text-xl font-semibold">New {kind}</h2><p className="text-sm text-muted-foreground">{patient.fullName} · {patient.patientNo}</p></div><Button variant="ghost" onClick={onClose}>Close</Button></div><div className="mt-5 grid gap-3 sm:grid-cols-2">
    ...
    {kind === 'prescription' ? <><Field label="Prescribing doctor"><Select value={String(values.doctorId)} onChange={(event) => set('doctorId', event.target.value)}><option value="">Select doctor</option>{staff.filter((member) => member.role === 'ADMIN').map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</Select></Field><Field label="Prescription date"><Input type="datetime-local" value={String(values.prescribedAt)} onChange={(event) => set('prescribedAt', event.target.value)} /></Field><TextField label="Consultation / treatment provided" name="consultationSummary" values={values} set={set} /><TextField label="Diagnosis / clinical impression" name="diagnosis" values={values} set={set} /><div className="sm:col-span-2 rounded-xl border p-4"><div><h3 className="font-semibold">Add medicines</h3><p className="text-sm text-muted-foreground">Search the catalogue and click a result to add it straight to the list, then complete its directions below. You can keep searching and adding more medicines right away.</p></div><div className="relative mt-4"><Field label="Search medicine"><div className="relative"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input className="pl-9" value={medicineSearch} onChange={(event) => setMedicineSearch(event.target.value)} placeholder="Type medicine, cream, lotion, tablet..." /></div></Field>{medicineSearch.trim() ? <div className="mt-1 max-h-52 overflow-y-auto rounded-md border bg-surface p-1 shadow-sm">{medicineQuery.isLoading ? <p className="p-3 text-sm text-muted-foreground">Searching medicines...</p> : medicines.length ? medicines.map((medicine) => <button key={medicine.id} type="button" className="flex w-full items-center justify-between rounded px-3 py-2 text-left text-sm hover:bg-muted" onClick={() => selectMedicine(medicine.id)}><span className="font-medium">{medicine.name}{medicine.strength ? ` · ${medicine.strength}` : ''}</span><span className="text-xs text-muted-foreground">{medicine.form || medicine.genericName || 'Medicine'}</span></button>) : <p className="p-3 text-sm text-muted-foreground">No catalogue medicine found. Use the manual medicine field below.</p>}</div> : null}</div><div className="my-4 flex items-center gap-3 text-xs text-muted-foreground"><span className="h-px flex-1 bg-border" />OR ENTER MANUALLY<span className="h-px flex-1 bg-border" /></div><div className="flex flex-wrap items-end gap-3"><div className="min-w-[220px] flex-1"><Field label="Medicine not in catalogue"><Input value={manualMedicineName} onChange={(event) => setManualMedicineName(event.target.value)} placeholder="Write medicine name exactly as it should print" /></Field></div><Button type="button" variant="secondary" disabled={!manualMedicineName.trim()} onClick={addManualMedicine}><Plus className="size-4" />Add medicine</Button></div>{prescriptionItems.length ? <div className="mt-5 space-y-3"><h4 className="text-sm font-semibold">Medicines on prescription ({prescriptionItems.length})</h4>{prescriptionItems.map((item, index) => <div key={`${item.medicineId ?? item.medicineName}-${index}`} className="rounded-lg border bg-muted/20 p-3"><div className="flex items-start justify-between gap-3"><div className="font-semibold text-primary">{index + 1}. {item.medicineName || 'Unnamed medicine'}</div><Button type="button" variant="ghost" className="h-8 w-8 px-0" onClick={() => removeItem(index)}><Trash2 className="size-4" /></Button></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Field label="Strength"><Input value={item.strength} onChange={(event) => updateItem(index, 'strength', event.target.value)} placeholder="e.g. 10 mg or 2%" /></Field><PrescriptionSelect label="Dose / application" value={item.dosage} placeholder="Select dose" options={['1 tablet', '1/2 tablet', '1 capsule', '5 ml', '10 ml', 'Apply a thin layer', 'Apply 2-3 drops', 'One fingertip unit', 'Use as face wash', 'Use as shampoo', 'As directed']} onChange={(value) => updateItem(index, 'dosage', value)} /><PrescriptionSelect label="Frequency" value={item.frequency} placeholder="Select frequency" options={['Once daily', 'Twice daily', 'Three times daily', 'Every morning', 'Every night', 'Morning and evening', 'Alternate days', 'Once weekly', 'As needed', 'As directed']} onChange={(value) => updateItem(index, 'frequency', value)} /><PrescriptionSelect label="Duration" value={item.duration} placeholder="Select duration" options={['3 days', '5 days', '7 days', '10 days', '14 days', '3 weeks', '1 month', '2 months', '3 months', 'Until next review', 'As directed']} onChange={(value) => updateItem(index, 'duration', value)} /><PrescriptionSelect label="Route" value={item.route} placeholder="Select route" options={['Oral', 'Topical', 'Scalp', 'Face', 'Body', 'Nail', 'Intralesional', 'As directed']} onChange={(value) => updateItem(index, 'route', value)} /><PrescriptionSelect label="Food / time instruction" value={item.timing} placeholder="Select timing" options={['Before food', 'After food', 'With food', 'Empty stomach', 'Morning', 'Bedtime', 'Morning and evening', 'After bathing', 'As directed']} onChange={(value) => updateItem(index, 'timing', value)} /><label className="grid gap-1 text-sm sm:col-span-2 lg:col-span-3"><span className="font-semibold text-foreground">Special instructions</span><textarea className="min-h-16 rounded-md border border-border bg-surface px-3 py-2" value={item.instructions} onChange={(event) => updateItem(index, 'instructions', event.target.value)} placeholder="Application area, precautions, quantity, or tapering instructions" /></label></div></div>)}</div> : null}</div><Area label="General instructions" name="instructions" values={values} set={set} /><Area label="Precautions" name="precautions" values={values} set={set} /></> : null}
