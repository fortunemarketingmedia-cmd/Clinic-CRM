...
import { AlertTriangle, BellRing, CalendarClock, CheckCircle2, Phone, Search } from 'lucide-react';
...
export function NotificationsView() {
  ...
  const [tab, setTab] = useState<'ACTION' | 'UPCOMING' | 'COMPLETED'>('ACTION');
  ...
  const visible = rows.filter((row) => {
    ...
    const matchesTab = tab === 'COMPLETED' ? completed : tab === 'ACTION' ? !completed && alertDue : !completed && !alertDue;
  ...
  const actionCount = rows.filter((row) => !['COMPLETED', 'CANCELLED'].includes(row.status) && new Date(row.alertAt).getTime() <= now).length;
  ...
  return <section className="space-y-5">
    <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Work inbox</p><h1 className="mt-1 text-2xl font-semibold">Notifications & Reminders</h1><p className="text-sm text-muted-foreground">Follow-up alerts and assigned tasks, refreshed automatically every 30 seconds.</p></div>
    <div className="grid gap-3 sm:grid-cols-3"><Metric icon={AlertTriangle} label="Action required" value={actionCount} urgent={actionCount > 0} /><Metric icon={CalendarClock} label="Upcoming" value={upcomingCount} /><Metric icon={CheckCircle2} label="Completed" value={completedCount} /></div>
    <Card className="p-3"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><SegmentedTabs tabs={[{ label: 'Action required', value: 'ACTION', count: actionCount }, { label: 'Upcoming', value: 'UPCOMING', count: upcomingCount }, { label: 'Completed', value: 'COMPLETED', count: completedCount }]} value={tab} onChange={setTab} /><label className="relative w-full lg:max-w-sm"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search reminders" /></label></div></Card>
    ...
    <Card className="overflow-hidden p-0">{followUps.isLoading || tasks.isLoading ? <RowsSkeleton rows={7} /> : followUps.isError || tasks.isError ? <Empty text="Reminders could not be loaded." /> : !visible.length ? <Empty text="No reminders in this view." /> : <div className="divide-y divide-border">{visible.map((row) => { const overdue = !['COMPLETED', 'CANCELLED'].includes(row.status) && new Date(row.dueAt).getTime() < now; return <article key={`${row.kind}-${row.id}`} className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-semibold text-primary">{row.kind === 'FOLLOW_UP' ? 'FOLLOW-UP' : 'TASK'}</span>{overdue ? <span className="rounded-full bg-red-50 px-2 py-0.5 text-[11px] font-semibold text-red-700">OVERDUE</span> : null}<h2 className="font-medium">{row.title}</h2></div><p className="mt-1 text-sm text-muted-foreground">{row.personName ? `${row.personName} · ` : ''}{row.detail}</p><p className="mt-1 text-xs text-muted-foreground">{row.branch} · Assigned to {row.assignedTo} · Alert {formatDateTime(row.alertAt)}</p></div><div className="flex flex-wrap gap-2">{row.mobile ? <a className="inline-flex h-10 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium hover:bg-muted" href={`tel:${row.mobile}`}><Phone className="size-4" />Call</a> : null}{row.leadId ? <Link className="inline-flex h-10 items-center rounded-md border border-border px-3 text-sm font-medium hover:bg-muted" href={`/leads/${row.leadId}`}>Open lead</Link> : null}{!['COMPLETED', 'CANCELLED'].includes(row.status) ? <Button onClick={() => setSelected(row)}>Action taken</Button> : null}</div></article>; })}</div>}</Card>
  </section>;
}
